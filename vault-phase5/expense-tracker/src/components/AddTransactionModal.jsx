...
import { updateCategoryLearning, getMerchantCategory } from '../services/merchantLearningService';
...
export default function AddTransactionModal() {
  ...
  const handleMerchantBlur = async () => {
    if (merchant && !category) {
      // Tier 1: check what the user has taught us about this merchant
      const learned = await getMerchantCategory(merchant);
      if (learned && learned !== 'other') {
        setCategory(learned);
        return;
      }
      // Tier 2: fall back to the static keyword map
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing) {
        ...
        // If the user changed the category, teach the learning engine
        if (merchant && category && category !== 'other') {
          await updateCategoryLearning(merchant, category);
        }
        showNotif('Transaction updated ✓');
      } else {
        await addTxn(data);
        // On every save with a named merchant, record the merchant→category pair
        if (merchant && category && category !== 'other') {
          await updateCategoryLearning(merchant, category);
        }
