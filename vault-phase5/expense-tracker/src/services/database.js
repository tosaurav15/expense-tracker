...
const DB_CONFIG = {
  ...
  version: 2,              // ← bumped from 1→2 to add merchant_learning store (Phase 5)
  stores: {
    transactions: {
      keyPath: 'id',
      indexes: [
    ...
    // ── NEW IN v2: Merchant Learning Store ──────────────────
    // Stores what category each merchant belongs to, learned from
    // user corrections over time.
    //
    // Each record shape:
    // {
    //   id:         'amazon',       ← normalised merchant name (keyPath)
    //   category:   'shopping',     ← learned category
    //   confidence: 3,              ← times user confirmed / corrected
    //   source:     'user'|'auto',  ← how this entry was created
    //   lastSeen:   '2026-03-15',   ← ISO date of last transaction
    //   variants:   ['AMAZON', 'Amazon Marketplace'],  ← name aliases seen
    // }
    merchant_learning: {
      keyPath: 'id',
      indexes: [
