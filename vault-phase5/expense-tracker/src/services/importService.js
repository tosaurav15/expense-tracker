...
import { getMerchantCategory, saveMerchantCategory } from './merchantLearningService.js';
...
export async function saveImportedTransactions(transactions, onProgress) {
  ...
  for (let i = 0; i < transactions.length; i++) {
    ...
    try {
      // ── Phase 5: Enrich category from merchant learning engine ──
      // If the static map gave 'other', check if the user has taught us better
      if (txn.merchant && (!txn.category || txn.category === 'other')) {
        const learned = await getMerchantCategory(txn.merchant);
        if (learned && learned !== 'other') {
          txn = { ...txn, category: learned };
        }
      }

      await addTransaction(txn);

      // Record merchant→category so future imports also benefit
      if (txn.merchant && txn.category && txn.category !== 'other') {
        await saveMerchantCategory(txn.merchant, txn.category, 'auto');
      }
