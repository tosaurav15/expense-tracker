    ...
    >
      <div className="flex items-center justify-around px-2 pt-2 pb-1 max-w-lg mx-auto relative">
        {NAV_ITEMS.map((item, idx) => {
          ...
          // Insert FAB between History and Analytics
          const fab = idx === 2 ? (
            <button
              key="fab"
              onClick={() => setShowAddModal(true)}
              className="relative -mt-8 flex items-center justify-center w-14 h-14 rounded-full tap-active"
              style={{
                background: 'linear-gradient(135deg, #F0A500, #FFD166)',
                boxShadow: '0 0 24px rgba(240, 165, 0, 0.5), 0 4px 16px rgba(0,0,0,0.4)',
              }}
              aria-label="Add transaction"
            >
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#080C18" strokeWidth="2.5" strokeLinecap="round">
          ...
          return (
            <React.Fragment key={item.id}>
              ...
              <button
                ...
                className="flex flex-col items-center gap-1 px-3 py-1 min-w-[52px] tap-active"
                style={{ color: isActive ? '#F0A500' : '#5A6A8A' }}
              >
